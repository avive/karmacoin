...
use base::karma_coin::karma_coin_core_types::{ExecutionResult, FeeType, SignedTransaction, TransactionEvent, User};
use db::db_service::{DatabaseService, DataItem, ReadItem, WriteItem};
use crate::services::db_config_service::{MOBILE_NUMBERS_COL_FAMILY, NICKS_COL_FAMILY, RESERVED_NICKS_COL_FAMILY, TRANSACTIONS_COL_FAMILY, USERS_COL_FAMILY};
use prost::Message;
...
pub(crate) async fn _process_transaction(transaction: &SignedTransaction, tokenomics: &Tokenomics, event: &mut TransactionEvent) -> Result<()> {
    ...
    if !apply_subsidy && tx_fee >= user.balance {
        ...
        return Err(anyhow!("tx fee is greater than user balance no tx fee subsidy is applied"));
    }
    ...
    let evidence_account_id = evidence.account_id.ok_or_else(|| anyhow!("missing account id in verifier data"))?;
