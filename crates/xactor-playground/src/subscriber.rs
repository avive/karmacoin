...
pub async fn subscriber() -> std::io::Result<()> {
    let parent_addr = SubscriberParent::new().await.start().await.unwrap();
    println!("Running...");
    sleep(Duration::from_secs(5)).await; // let it work for a while
    println!("Stopping...");

    let _ = parent_addr.send(Stop);
    parent_addr.wait_for_stop().await;
    println!("SubscriberParent stopped.");
    Ok(())
}
...
impl Handler<InitializeChildSubscribers> for SubscriberParent {
    async fn handle(&mut self, _ctx: &mut Context<Self>, _msg: InitializeChildSubscribers) {
        ...
        let children_addr_vec =
            children_unstarted_actors_vec.map(|actor| async { actor.start().await.unwrap() });
...
#[async_trait::async_trait]
impl Handler<Stop> for SubscriberParent {
    async fn handle(&mut self, ctx: &mut Context<Self>, _msg: Stop) {
        let _ = self.message_producer.send(Stop);
        println!("Stopping SubscriberParent");
        ctx.stop(None);
    }
}
...
impl Handler<SubscribeToProducer> for MessageProducer {
    async fn handle(&mut self, _ctx: &mut Context<Self>, msg: SubscribeToProducer) {
        println!("Received Subscription Request");
        self.subscribers.push(msg.sender);
    }
}

#[async_trait::async_trait]
impl Handler<Stop> for MessageProducer {
    async fn handle(&mut self, ctx: &mut Context<Self>, _msg: Stop) {
        println!("Stopping MessageProducer");
        ctx.stop(None);
    }
}
